...
from pydantic import PrivateAttr # Import PrivateAttr for the best practice

load_dotenv()

# --- Tool Blueprints (Unchanged) ---
...
def external_rest(auth_token: str, url: str) -> str:
    """Connect to an external REST API that requires an authentication token."""
    pass
...
class AgentBuilder(Agent):
    ...
    """
    # ✨ FIX: Declare instance variables as "private" using a leading underscore.
    # This prevents Pydantic from trying to validate them as model fields.
    ...
    def __init__(self, model: GenerativeModel):
        ...
        # ✨ FIX: Assign to the private attributes.
        self._model = model
    ...
    def run(self):
        ...
        3.  If you are missing information for a tool's parameters (e.g., a URL, a recipient's email), you MUST ask the user a clear, direct question to get the missing information. Do NOT make up placeholders.
        ...
        5.  After adding a tool to the configuration, confirm with the user and ask what to do next.
        6.  When the user indicates they are finished adding tools, call the `finalize_configuration` function. You will need to ask for a name and description for the agent.
        """
        # ✨ FIX: Use the private attribute `_model`
        self._model = genai.GenerativeModel(
            model_name='gemini-2.5-flash',
            # ✨ FIX: Use the private attribute `_available_tools`
        ...
        while True:
            ...
            if response_part.function_call:
                ...
                if tool_name == "finalize_configuration":
                    ...
                    # ✨ FIX: Use the private attribute `_configured_tools`
    ...
    def _handle_document_tool(self, tool_args: dict):
        ...
        try:
            ...
            if os.path.exists(local_path):
                ...
                # ✨ FIX: Use the private attribute `_configured_tools`
                self._configured_tools.append(tool_config)
                print(f"🤖 Agent Builder: ✅ OK, I've configured the document tool using '{local_path}'. What's next?")
            else:
                print(f"🤖 Agent Builder: ❌ Error: File not found at '{local_path}'. Please try configuring the tool again.")
        except Exception as e:
            print(f"🤖 Agent Builder: ❌ An error occurred: {e}")
...
if __name__ == "__main__":
    try:
        ...
        # and a .env file with your GOOGLE_API_KEY
