...
function App() {
  ...
  const handleConfigureTool = async (e) => {
    ...
    // currentToolParams already holds both file_name and file_content if it's document_tool
  ...
  // Handler for file input change specifically for document_tool
  const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        // Store both the file name and its content
        setCurrentToolParams(prevParams => ({
          ...prevParams,
          file_name: file.name,
          file_content: event.target.result // Content of the file as text
        }));
      };
            ...
            {toolInfo.parameters.length > 0 ? (
              toolInfo.parameters.map(param => (
                <div key={param.name} className="param-input">
                  ...
                  {/* Special handling for document_tool's file_name parameter */}
                  {param.name === 'file_name' && toolInfo.tool_name === 'document_tool' ? (
                      <>
                        ...
                        {currentToolParams.file_name && (
                            <p className="selected-file-name">Selected: {currentToolParams.file_name}</p>
                        )}
                        <small>Select a file. Its content will be read for demo purposes.</small>
                      </>
                  ) : (
